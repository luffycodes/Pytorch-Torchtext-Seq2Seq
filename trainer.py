        ...
        for epoch in range(self.num_epoch):
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                if i % 100 == 0 and i != 0:
                    ...
                    self.console_logger.debug("encoder : all_weights[0][0].data[0] %1.3f", torch.sum(self.model.encoder.gru.all_weights[0][0].data[0]))
                    self.console_logger.debug("encoder : all_weights[0][0].grad[0] %1.8f", torch.sum(self.model.encoder.gru.all_weights[0][0].grad[0].data))
                    self.console_logger.debug("decoder : all_weights[0][0].data[0] %1.3f", torch.sum(self.model.decoder.gru.all_weights[0][0].data[0]))
                    self.console_logger.debug("decoder : all_weights[0][0].grad[0] %1.8f", torch.sum(self.model.decoder.gru.all_weights[0][0].grad[0].data))
